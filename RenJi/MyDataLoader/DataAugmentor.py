...
class DataAugmentor3D():
    ...
    def RandomParameter(self, param):
        import random
        # if isinstance(param, list) and len(param) == output:
    ...
    def SetParameter(self, parameter_dict):
        if 'stretch_x' in parameter_dict: self.stretch_x = self.RandomParameter(parameter_dict['stretch_x'])
        if 'stretch_y' in parameter_dict: self.stretch_y = self.RandomParameter(parameter_dict['stretch_y'])
        if 'stretch_z' in parameter_dict: self.stretch_z = self.RandomParameter(parameter_dict['stretch_z'])
        if 'shear' in parameter_dict: self.shear = self.RandomParameter(parameter_dict['shear'])
        if 'rotate_z_angle' in parameter_dict: self.rotate_z_angle = self.RandomParameter(parameter_dict['rotate_z_angle'])
        if 'rotate_x_angle' in parameter_dict: self.rotate_x_angle = self.RandomParameter(parameter_dict['rotate_x_angle'])
        if 'shift_x' in parameter_dict: self.shift_x = self.RandomParameter(parameter_dict['shift_x'])
        if 'shift_y' in parameter_dict: self.shift_y = self.RandomParameter(parameter_dict['shift_y'])
        if 'shift_z' in parameter_dict: self.shift_z = self.RandomParameter(parameter_dict['shift_z'])
        ...
        if 'slice_flip' in parameter_dict: self.slice_flip = self.RandomParameter(parameter_dict['slice_flip'])

        if 'bias_center' in parameter_dict: self.bias_center = self.RandomParameter(parameter_dict['bias_center'])
        if 'bias_drop_ratio' in parameter_dict: self.bias_drop_ratio = self.RandomParameter(parameter_dict['bias_drop_ratio'])
        if 'noise_sigma' in parameter_dict: self.noise_sigma = self.RandomParameter(parameter_dict['noise_sigma'])
        if 'factor' in parameter_dict: self.factor = self.RandomParameter(parameter_dict['factor'])
        if 'gamma' in parameter_dict: self.gamma = self.RandomParameter(parameter_dict['gamma'])

        if 'elastic' in parameter_dict: self.elastic = self.RandomParameter(parameter_dict['elastic'])
        if 'is_debug' in parameter_dict: self.is_debug = self.RandomParameter(parameter_dict['is_debug'])
    ...
    def _Shift3DImage(self, data):
        x, y, z = data.shape
        non = lambda s: int(s) if int(s) < 0 else None
        mom = lambda s: max(0, int(s))

        shifted_data = np.zeros_like(data)
        shifted_data[mom(self.shift_x *x):non(self.shift_x *x), mom(self.shift_y * y):non(self.shift_y * y), mom(self.shift_z * z):non(self.shift_z * z)] = \
            data[mom(-self.shift_x *x):non(-self.shift_x *x), mom(-self.shift_y * y):non(-self.shift_y * y), mom(-self.shift_z * z):non(-self.shift_z * z)]
    ...
    def _ElasticAugment(self, data, num_controlpoints=4, std_deformation_sigma=15, proportion_to_augment=0.5,
                        spatial_rank=0, interp_order=3):

        data = list(data.transpose((2, 0, 1)))
        if not isinstance(interp_order, list):
            interp_order = [interp_order for index in data]
        if spatial_rank == 0:
            spatial_rank = data[0].ndim

        data = [np.expand_dims(index, axis=-1) for index in data]

        rand_pairs = RandomElasticDeformation(num_controlpoints, std_deformation_sigma, proportion_to_augment, spatial_rank)

        rand_pairs.randomise(data[0].shape)
        augment_data_list = []
        for data, interp in zip(data, interp_order):
            augment_data_list.append(rand_pairs.apply_transformation_3d(data, interp))

        return np.array([np.squeeze(data, axis=-1) for data in augment_data_list]).transpose((1, 2, 0))
    ...
    def _AddBias(self, data):
        field = self._BiasField(data[..., 0].shape)
        ...
        min_value = data.min()
        data_list = [np.multiply(field, data[..., index] - min_value) + min_value for index in range(data.shape[-1])]
        return np.array(data_list).transpose((1, 2, 0))

    def _Noise(self, data):
        # TODO: how to add noise
        if data.max() - data.min() < 1e-6:
            return data

        if data.max() < 0.:
            new_data = data - data.min()
            noise = np.random.normal(0., self.noise_sigma * new_data.max(), data.shape)
        else:
    ...
    def Execute(self, source_data, parameter_dict={}, interpolation='linear', is_clear=False, is_not_roi=True):
        ...
        if min(source_data.shape) != source_data.shape[-1]:
        ...
        if is_not_roi:
            ...
            target_data = self._Noise(target_data)
            target_data = self._Contrast(target_data)
            target_data = self._Gamma(target_data)
        if self.elastic:
            target_data = self._ElasticAugment(target_data)

        if is_clear:
            self.ClearParameters()
        if is_transpose:
            return target_data.transpose(2, 0, 1)
        else:
            return target_data
...
class DataAugmentor2D():
    ...
    def _BiasField(self, input_shape):
        ...
        center_x = int(input_shape[0] * self.bias_center)
        center_y = int(input_shape[1] * self.bias_center)
        max_x = max(input_shape[0] - center_x, center_x)
        max_y = max(input_shape[1] - center_y, center_y)
...
def main():
    from MeDIT.UsualUse import LoadImage, Imshow3DArray, Normalize01
    # Test ElasticAugment
    import matplotlib.pyplot as plt
    image, _, data = LoadImage(r'd:\Data\PCa-Detection\JSPH-IndenpendentTest\CA\BIAN ZHONG BEN\t2_Resize.nii')
    _, _, roi = LoadImage(r'd:\Data\PCa-Detection\JSPH-IndenpendentTest\CA\BIAN ZHONG BEN\prostate_roi_25D.nii.gz', dtype=int)
