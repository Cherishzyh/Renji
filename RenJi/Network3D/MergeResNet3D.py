...
class MergeResNet(nn.Module):
    ...
    def __init__(self, block, layers, block_inplanes, n_input_channels=3, n_classes=400):
        ...
        self.fc1 = nn.Sequential(nn.Linear(block_inplanes[3] * block.expansion * 2, 64 * block.expansion),
                                 nn.Dropout(0.5),
                                 nn.ReLU(inplace=True))
        self.fc2 = nn.Linear(64 * block.expansion, n_classes)

    def forward(self, x_1, x_2):
        x_1 = self.resnet_a(x_1)
        x_2 = self.resnet_b(x_2)
...
def GenerateModel(model_depth, **kwargs):
    ...
    if model_depth == 10:
        model = MergeResNet(BasicBlock, [1, 1, 1, 1], get_inplanes(), **kwargs)
    elif model_depth == 18:
        model = MergeResNet(BasicBlock, [2, 2, 2, 2], get_inplanes(), **kwargs)
    elif model_depth == 34:
        model = MergeResNet(BasicBlock, [3, 4, 6, 3], get_inplanes(), **kwargs)
    elif model_depth == 50:
        model = MergeResNet(Bottleneck, [3, 4, 6, 3], get_inplanes(), **kwargs)
    elif model_depth == 101:
        model = MergeResNet(Bottleneck, [3, 4, 23, 3], get_inplanes(), **kwargs)
    elif model_depth == 152:
        model = MergeResNet(Bottleneck, [3, 8, 36, 3], get_inplanes(), **kwargs)
    elif model_depth == 200:
        model = MergeResNet(Bottleneck, [3, 24, 36, 3], get_inplanes(), **kwargs)

    return model


if __name__ == '__main__':
    # model = MergeResNet(Bottleneck, [3, 4, 6, 3], get_inplanes(), n_input_channels=1, conv1_t_size=7, conv1_t_stride=1,
    #              no_max_pool=False, shortcut_type='B', widen_factor=1.0, n_classes=4)
    model = GenerateModel(50, n_input_channels=1, n_classes=1)
