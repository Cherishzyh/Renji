...
class ConfigInterpretor:
    ...
    def LoadModelConfig(self, config_path):
        ...
        self._config['out_channel'] = int(cf.get("net_structure", "out_channel"))
    ...
    def CropDataShape(self, data, raw_resolution, center_point=[-1, -1, -1]):
        ...
        if self._config['model_dimension'] == 2:
            target_resolution = [raw_resolution[0], self._config['resolution_x'], self._config['resolution_y']]
            final_data_shape = [new_data.shape[0], self._config['input_x'], self._config['input_y']]
            self._log = np.zeros((9,))

        elif self._config['model_dimension'] == 3:
            target_resolution = [self._config['resolution_z'], self._config['resolution_x'],
                                 self._config['resolution_y']]
            final_data_shape = [self._config['input_z'], self._config['input_x'],
                                self._config['input_y']]
        ...
        if final_data_shape[1] < new_data_shape[1]:  # crop new_data in row direction
            self._log[3] = 1
            if self._center_point[1] == -1:
                row_start = new_data_shape[1] // 2 - final_data_shape[1] // 2
            else:
                row_start = self._center_point[1] - final_data_shape[1] // 2
            row_end = row_start + final_data_shape[1]
            new_data = new_data[:, row_start:row_end, :]

        elif final_data_shape[1] > new_data_shape[1]:  # wrap zeros on new_data in row direction
            self._log[3] = 2
            row_start = final_data_shape[1] // 2 - new_data_shape[1] // 2
            row_end = row_start + new_data_shape[1]
            ...
            row_start = 1
            row_end = final_data_shape[1]
        self._log[4:6] = [row_start, row_end]


        if final_data_shape[2] < new_data_shape[2]:  # crop new_data in col direction
            self._log[6] = 1
            if self._center_point[2] == -1:
                col_start = new_data_shape[2] // 2 - final_data_shape[2] // 2
            else:
                col_start = self._center_point[2] - final_data_shape[2] // 2
            col_end = col_start + final_data_shape[2]
            new_data = new_data[:, :, col_start:col_end]

        elif final_data_shape[2] > new_data_shape[2]:  # wrap zeros on new_data in col direction
            self._log[6] = 2
            col_start = final_data_shape[2] // 2 - new_data_shape[2] // 2
            col_end = col_start + new_data_shape[2]
            ...
            col_start = 0
            col_end = final_data_shape[2]
        self._log[7:9] = [col_start, col_end]

        if self._config['model_dimension'] == 3:
            self._log[9] = 1
            if final_data_shape[0] < new_data_shape[0]:  # crop new_data in slice direction
                if self.__curslice != None:
                    slice_start = self.__curslice - 8
                    slice_end = self.__curslice + 8
                else:
                    slice_start = new_data_shape[0] // 2 - final_data_shape[0] // 2
                    slice_end = slice_start + final_data_shape[0]

                new_data = new_data[slice_start:slice_end, :, :]
            elif final_data_shape[0] > new_data_shape[0]:  # wrap zeros on new_data in slice direction
                self._log[9] = 2
                slice_start = final_data_shape[0] // 2 - new_data_shape[0] // 2
                slice_end = slice_start + new_data_shape[0]
                ...
                slice_start = 0
                slice_end = final_data_shape[0]
            self._log[10:12] = [slice_start, slice_end]

        self._log = np.asarray(self._log, dtype=int)
        self._log = np.asarray(self._log, dtype=int)
    ...
    def RecoverDataShape(self, data, raw_resolution):
        ...
        if self._log[3] == 2:  # crop new_data in row direction
            ...
            temp_new_data = np.zeros((new_data_shape[0], hidden_data_shape[1], new_data_shape[2]))
            temp_new_data[:, row_start:row_end, :] = new_data
            new_data = temp_new_data
            del temp_new_data
        ...
        if self._log[6] == 2:  # crop new_data in col direction
            ...
            temp_new_data = np.zeros((new_data_shape[0], hidden_data_shape[1], hidden_data_shape[2]))
            temp_new_data[:, :, col_start:col_end] = new_data
            new_data = temp_new_data
            del temp_new_data
    ...
    def GetResolution(self):
        ...
        if self.GetDimension() == 3:
            return [self._config['resolution_z'], self._config['resolution_x'], self._config['resolution_y']]

    def GetShape(self):
        if self.GetDimension() == 2:
            return [self._config['input_x'], self._config['input_y']]
        if self.GetDimension() == 3:
            return [self._config['input_z'], self._config['input_x'], self._config['input_y']]

    def GetName(self):
        return self._config['name']

    def GetChannel(self):
        return self._config['num_channel'], self._config['out_channel']
...
if __name__ == '__main__':
    bi = BaseImageOutModel()
    bi.LoadConfigAndModel(r'/home/zhangyihong/Documents/RenJi/SuccessfulModel/UNet_1118')